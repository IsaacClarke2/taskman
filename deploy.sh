...
setup_nginx_http() {
    log STEP "[8/9] Setting up Nginx (HTTP for certbot)..."
    ...
    # Create temporary HTTP-only Nginx config for certbot verification
    cat > "$NGINX_CONF" << EOF
# Telegram AI Business Assistant - HTTP only (for SSL verification)
server {
    listen 80;
    listen [::]:80;
...
EOF
    ...
    if nginx -t 2>&1; then
        nginx -s reload 2>/dev/null || systemctl reload nginx || systemctl restart nginx
        log INFO "Nginx HTTP configured"
    else
        log WARN "Nginx config test failed"
    fi
}

setup_nginx_https() {
    log STEP "Configuring Nginx with HTTPS..."
...
EOF
    ...
    if nginx -t 2>&1; then
        nginx -s reload 2>/dev/null || systemctl reload nginx || systemctl restart nginx
        log INFO "Nginx HTTPS configured"
    else
        log ERROR "Nginx HTTPS config test failed"
        nginx -t
    fi
}
...
setup_ssl() {
    ...
    if [ -d "/etc/letsencrypt/live/${DOMAIN}" ]; then
        ...
        # Get certificate using webroot method (nginx is already running with HTTP config)
        certbot certonly --webroot -w /var/www/certbot \
            -d "${DOMAIN}" -d "www.${DOMAIN}" \
            --non-interactive --agree-tos -m "${EMAIL}" \
            2>&1 | tee -a "$LOG_FILE" || {
            log WARN "Certbot webroot failed, trying standalone..."
            ...
            certbot certonly --standalone \
                ...
                log WARN "You can try manually: certbot certonly --standalone -d ${DOMAIN}"
            }
            # Start nginx again
    ...
    # If certificate exists, configure HTTPS in nginx
    if [ -d "/etc/letsencrypt/live/${DOMAIN}" ]; then
        setup_nginx_https
    fi
