...
function ZoomIntegrationContent() {
  ...
  const startOAuthFlow = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/integrations/zoom/auth`, {
        headers: {
  ...
  const handleCallback = async (code: string, state: string | null) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/integrations/zoom/callback`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok) {
        const data = await response.json()
        throw new Error(data.detail || 'Failed to connect Zoom')
      }
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
        <div className="mb-6">
          <svg className="w-16 h-16 mx-auto" viewBox="0 0 24 24">
            <path fill="#2D8CFF" d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0z"/>
            <path fill="#fff" d="M16.5 8.5v3.5l2.5-2v5l-2.5-2v3.5c0 .55-.45 1-1 1h-8c-.55 0-1-.45-1-1v-7c0-.55.45-1 1-1h8c.55 0 1 .45 1 1z"/>
          </svg>
        </div>
        ...
        {status === 'loading' && (
          <div>
            ...
            <p className="text-gray-600">Redirecting to Zoom...</p>
          </div>
        )}
        ...
        {status === 'connecting' && (
          <div>
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-4"></div>
            <p className="text-gray-600">Connecting your Zoom account...</p>
          </div>
        )}
